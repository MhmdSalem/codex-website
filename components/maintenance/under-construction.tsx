...
export function UnderConstruction({ message }: Props) {
  ...
  return (
    <main
      dir="rtl"
      className="relative min-h-screen overflow-hidden bg-background text-foreground grid place-items-center px-6"
    >
      {/* ── Background layers ───────────────────────────────────────────── */}
      <div
        aria-hidden="true"
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto text-center">
        ...
        >
          <motion.div
            className="absolute -inset-8 rounded-full bg-gold/20 blur-2xl"
          ...
          <div className="relative">
            <Image
              ...
              height={120}
              priority
              className="w-24 h-24 sm:w-28 sm:h-28 rounded-3xl shadow-[0_20px_60px_rgba(217,119,87,0.35)]"
            />
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-gold/30 bg-background-surface/40 backdrop-blur-md mb-8"
        >
          <motion.div
        ...
        {/* Arabic headline — primary */}
        <motion.h1
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, delay: 0.3, ease: [0.16, 1, 0.3, 1] }}
          className="font-display font-bold text-balance leading-[0.95] text-[clamp(2.5rem,7vw,5.5rem)]"
        >
          <span className="text-gradient-fade">الموقع تحت </span>
          <span className="text-gradient-gold italic">الصيانة</span>
        </motion.h1>
        ...
        <motion.p
          ...
          className="mt-4 font-display italic text-foreground-muted text-base sm:text-lg tracking-wide"
        >
        ...
        >
          <p className="text-foreground-muted text-sm sm:text-base leading-relaxed text-balance">
            {m.ar}
          </p>
          <div className="my-4 mx-auto h-px w-16 bg-gradient-to-r from-transparent via-gold/40 to-transparent" />
          <p
            dir="ltr"
            className="text-foreground-muted/80 text-xs sm:text-sm leading-relaxed text-balance"
          >
            {m.en}
          </p>
        </motion.div>
        ...
        <motion.p
          ...
          className="mt-10 text-xs text-foreground-subtle"
        >
          © {new Date().getFullYear()} Codex · صُنع بإتقان في القاهرة
        </motion.p>
      </div>
