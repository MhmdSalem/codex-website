      ...
      <div className="container-wide relative">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-y lg:divide-y-0 divide-border rtl:divide-x-reverse border border-border rounded-3xl overflow-hidden bg-background-surface/30 backdrop-blur-sm">
          {t.items.map((item, i) => (
            ...
            >
              <div className="font-display font-bold text-5xl sm:text-6xl lg:text-7xl text-gradient-gold-static leading-none mb-3 flex items-baseline justify-center">
                <Counter value={item.value} suffix={item.suffix} duration={2000} />
              </div>
              <div className="text-xs sm:text-sm uppercase tracking-[0.2em] text-foreground-muted">
                {item.label}
              </div>
